...
namespace DfoServer.Game.Names
{
    internal sealed class NameInputRuleSet
    {
        ...
        private bool HasUnicodeRanges => _allowedUnicodeRanges.Count > 0;

        private bool HasInvalidCharRules =>
            _oneByteRanges.Count > 0 ||
            _oneByteValues.Count > 0 ||
            _twoByteRanges.Count > 0 ||
            _twoByteValues.Count > 0;
        ...
        {
            if (!HasInvalidCharRules)
                return !ContainsOnlyFallbackAllowedCharacters(text);
        ...
        private static NameInputRuleSet LoadFromPvf()
        {
            ...
            TryLoad("unicode range", () => ParseUnicodeRanges(ReadFirstExistingText(
                "etc/RestrictNameUnicodeRangeTable.etc",
                "etc/restrictnameunicoderangetable.etc",
                "Etc/RestrictNameUnicodeRangeTable.etc"), rules));
            EnsureDefaultUnicodeRanges(rules);

            FileLogger.Log(
                "[NameInputRuleSet] loaded unicodeRanges={0}, oneByteRanges={1}, oneByteValues={2}, twoByteRanges={3}, twoByteValues={4}, slang={5}, oneByteSkip={6}, twoByteSkip={7}",
                ...
                rules._oneByteSkipChars.Count,
                rules._twoByteSkipChars.Count);
            return rules;
        }
        ...
        private static void ParseUnicodeRanges(string content, NameInputRuleSet rules)
        {
            string section = null;
            ...
            foreach (var line in EnumerateRuleLines(content))
            {
                ...
                if (tag != null)
                {
                    FlushUnicodeRange(rangeValues, rules);
                ...
                var values = ParseInts(line);
                if (values.Count > 0)
                    rangeValues.Add(values[values.Count - 1]);
            }

            FlushUnicodeRange(rangeValues, rules);
        }

        private static void FlushUnicodeRange(List<int> values, NameInputRuleSet rules)
        {
            ...
            rules._allowedUnicodeRanges.Add(new Range(values[0], values[1]));
            values.Clear();
        }

        private static void EnsureDefaultUnicodeRanges(NameInputRuleSet rules)
        {
            if (rules == null || rules._allowedUnicodeRanges.Count > 0)
                return;

            // Built-in copy of etc/restrictnameunicoderangetable.etc used by the client:
            // digits, common CJK, uppercase ASCII, lowercase ASCII.
            ...
            FileLogger.Log("[NameInputRuleSet] using built-in restrictnameunicoderangetable ranges.");
        }
            ...
            foreach (var line in EnumerateRuleLines(content))
            {
                ...
                for (var i = 0; i < tokens.Length; i++)
                {
                    ...
                    if (token.Length > 0 && !int.TryParse(token, out _))
                        AddSlang(StripBackticks(token), rules, seen);
                }
            }
        }
        ...
        private static bool ContainsOnlyFallbackAllowedCharacters(string text)
        {
            for (var i = 0; i < text.Length; i++)
            {
                var ch = text[i];
                if (char.IsSurrogate(ch) || !IsFallbackAllowedNameCharacter(ch))
                    return false;
            }
        ...
        private static bool IsFallbackAllowedNameCharacter(char ch)
        {
            if ((ch >= '0' && ch <= '9') ||
                (ch >= 'A' && ch <= 'Z') ||
                (ch >= 'a' && ch <= 'z'))
