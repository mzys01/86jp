...
namespace DfoServer.Game.Inventory
{
    ...
    {
        private bool TryInsertPetInventoryEquipmentFallback(
        ...
        {
            assignedSlot = -1;
            if (!IsPetInventoryEquipmentMetadata(metadata, itemTemplateId))
                return false;

            if (!TryGetPetVisibleSlotRange(itemTemplateId, out var slotStart, out var slotEnd))
                return false;

            var targetSlot = _db.FindEmptySlot(connection, transaction, characterId, InventoryListType.Pet, slotStart, slotEnd);
            if (targetSlot < 0)
            {
                FileLogger.Log($"  [PetItemFallback] no empty pet inventory slot item=0x{itemTemplateId:X8} range={slotStart}-{slotEnd}");
                return false;
            }

            var value = IsCreatureItem(itemTemplateId)
                ? _db.NextPetSerialOrHandle(connection, transaction, characterId)
                : 0;
            InsertNormalizedPetInventoryItem(connection, transaction, characterId, (short)targetSlot, itemTemplateId, value);
            assignedSlot = (short)targetSlot;
            FileLogger.Log($"  [PetItemFallback] routed grant item=0x{itemTemplateId:X8} -> Pet slot {assignedSlot}");
            return true;
        }
...
WHERE item_uid = @uid;";
        ...
        private static bool IsPetInventoryEquipmentMetadata(ItemMetadata metadata, int itemTemplateId)
        {
        ...
        private void InsertNormalizedPetInventoryItem(
            SqliteConnection connection,
            SqliteTransaction transaction,
            int characterId,
            short slot,
