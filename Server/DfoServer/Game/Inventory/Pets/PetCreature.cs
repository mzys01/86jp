    ...
    public sealed partial class SqliteInventoryStore
    {
            ...
            out InventoryMoveResult result)
        {
                ...
                && request.DestinationSlotIndex == PetCreatureEquipSlot)
            {
                ...
                var petSerial = EnsurePetCreaturePersistentSerial(connection, transaction, _context.CharacterId, source);
                if (petSerial == 0)
                {
                ...
                var currentlyEquipped = LoadPetCreatureEquippedEntry(connection, transaction, _context.CharacterId);
                ...
                _auditLogger.WriteAuditLog(connection, transaction, _context.CharacterId, "move_pet_creature_equip", source, InventoryListType.Pet, source.SlotIndex, 1);
                ...
                result = CreatePetInventoryAckResult(request, request.MoveCount, source.SlotIndex);
                ...
                && IsPetArtifactEquipSlot(request.DestinationSlotIndex))
            {
                ...
                var currentlyEquipped = LoadPetEquippedEntry(connection, transaction, _context.CharacterId, request.DestinationSlotIndex);
                _db.DeleteItem(connection, transaction, source.ItemUid);
                DeleteSortItemLock(_context.CharacterId, connection, transaction, InventoryListType.Pet, source.SlotIndex);
                ...
                CleanupLegacyPetArtifactStorage(connection, transaction, _context.CharacterId, storageSlot);

                _auditLogger.WriteAuditLog(connection, transaction, _context.CharacterId, "move_pet_artifact_equip", source, InventoryListType.Pet, source.SlotIndex, 1);
            ...
            if (request.SourceListType == InventoryListType.Equipment
                ...
                && request.DestinationListType == InventoryListType.Pet)
            {
                var equipped = LoadPetCreatureEquippedEntry(connection, transaction, _context.CharacterId);
                if (!equipped.HasValue)
                {
                    FileLogger.Log("  [PetCreatureMove] UNEQUIP: slot 24 already empty");
                    ClearPetEquippedEntry(connection, transaction, _context.CharacterId, PetCreatureEquipSlot);
                    result = CreatePetInventoryAckResult(
                        request,
                        request.MoveCount,
                        ResolvePetUnequipSlotForRefresh(request.DestinationSlotIndex),
                        mutated: false);
                    result.PetCreatureStateChanged = true;
                ...
                var targetSlot = ResolvePetUnequipSlot(connection, transaction, request.DestinationSlotIndex);
                if (targetSlot < 0)
                {
                ...
                ClearPetCreatureRuntimeState(connection, transaction, _context.CharacterId);
                CleanupLegacyPetCreatureStorage(connection, transaction, _context.CharacterId, null);

                ClearPetEquippedEntry(connection, transaction, _context.CharacterId, PetCreatureEquipSlot);
                result = CreatePetInventoryAckResult(request, request.MoveCount, targetSlot);
                result.PetCreatureStateChanged = true;
                result.PetItemStateChanged = true;
                AddPetCreatureRefreshSlot(result, PetCreatureEquippedStorageSlot);
            ...
            out InventoryMoveResult result)
        {
            ...
            if (!equipped.HasValue)
            {
                ...
                ClearPetCreatureRuntimeState(connection, transaction, _context.CharacterId);
                result = CreatePetInventoryAckResult(request, request.MoveCount, request.SourceSlotIndex);
                result.PetCreatureStateChanged = true;
                AddPetCreatureRefreshSlot(result, PetCreatureEquippedStorageSlot);
            ...
            var targetSlot = ResolvePetUnequipSlot(connection, transaction, request.SourceSlotIndex);
            if (targetSlot < 0)
            {
            ...
            out InventoryMoveResult result)
        {
            ...
            var targetSlot = ResolvePetArtifactUnequipSlot(connection, transaction, requestedPetSlot);
            if (targetSlot < 0)
            {
        ...
        {
            var result = CreateMoveResult(request, moveValue32, mutated);
            result.SourceListType = InventoryListType.Equipment;
            result.SourceSlotIndex = (short)equipSlot;
            result.DestinationListType = InventoryListType.Pet;
            result.DestinationSlotIndex = (short)targetPetSlot;
            result.PetItemStateChanged = true;
            return result;
        }
        ...
        private int ResolvePetUnequipSlot(SqliteConnection connection, SqliteTransaction transaction, short requestedSlot)
        {
            if (requestedSlot >= PetInventorySlotStart
                && requestedSlot <= PetInventorySlotEnd
                && IsPetInventorySlotEmpty(connection, transaction, _context.CharacterId, requestedSlot))
                return requestedSlot;

            return _db.FindEmptySlot(connection, transaction, _context.CharacterId, InventoryListType.Pet, PetInventorySlotStart, PetInventorySlotEnd);
        }

        private int ResolvePetArtifactUnequipSlot(SqliteConnection connection, SqliteTransaction transaction, short requestedSlot)
        {
            ...
            return _db.FindEmptySlot(connection, transaction, _context.CharacterId, InventoryListType.Pet, PetEquipmentSlotStart, PetEquipmentSlotEnd);
        }
...
WHERE character_id = @cid
  ...
  AND slot_index = @slot;";
        ...
        public void RepairPetCreatureState()
        {
            using (var connection = _context.OpenConnection())
            using (var transaction = connection.BeginTransaction())
            {
                RepairPetCreatureState(connection, transaction, _context.CharacterId);
                transaction.Commit();
            }
        }
...
LIMIT 1;";
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    ...
                    if (raw == null)
                        return new PetCreatureEquippedEntry(itemId, 0);

                    try
                    {
                        var parsed = InvenItem.Parse(raw);
                        return new PetCreatureEquippedEntry(parsed.ItemId, unchecked((int)parsed.Value));
                    }
                    catch (Exception ex)
                    {
                        ...
                        return new PetCreatureEquippedEntry(itemId, serial);
                    }
                }
            }
        }
...
WHERE character_id = @cid
  AND rowid NOT IN (
      ...
      GROUP BY creature_key
        ...
        {
            public PetCreatureEquippedEntry(int itemId, int serial)
            {
                ItemId = itemId;
                Serial = serial;
            }
...
DO UPDATE SET item_id = @itemId,
        ...
        public AvatarInventoryItem LoadEquipmentItemForRefresh(short slotIndex)
        {
            using (var connection = _context.OpenConnection())
            {
                return LoadEquipmentItemForRefresh(connection, null, _context.CharacterId, slotIndex)
                    ?? new AvatarInventoryItem { SlotIndex = slotIndex };
            }
        }
