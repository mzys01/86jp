...
namespace DfoServer.Game.Inventory
{
    public sealed partial class SqliteInventoryStore
    {
            ...
            out PetCreatureRenameResult result)
        {
            ...
            {
                FileLogger.Log($"  [PetRename] failed: invalid request reason={failure}");
                return false;
            }
            ...
            {
                var creatureSerial = ResolveEquippedCreatureKey(connection, transaction, _context.CharacterId);
                var petItemTemplateId = LoadEquippedCreatureItemId(connection, transaction, _context.CharacterId);
                if (creatureSerial <= 0 || petItemTemplateId <= 0 || !IsCreatureItem(petItemTemplateId))
                {
                    FileLogger.Log($"  [PetRename] failed: no valid equipped pet character={_context.CharacterId} serial=0x{creatureSerial:X8} item=0x{petItemTemplateId:X8}");
                    return false;
                }
                ...
                UpdateCreatureName(connection, transaction, _context.CharacterId, creatureSerial, nameBytes);
