...
{
    public static class PetCreatureExperienceService
    {
        ...
        public static PetCreatureExperienceUpdate ApplyDungeonClearExperience(
            string databasePath,
            ...
            int consumedFatigue)
        {
            ...
            using (var connection = new SqliteConnection(connectionString))
            {
                ...
                {
                    var creatureKey = PetCreatureSatietyService.ResolveEquippedCreatureKey(connection, transaction, characterId);
                    if (creatureKey <= 0)
                        return PetCreatureExperienceUpdate.Noop(characterId);
                    ...
                    if (beforeLevel >= MaxCreatureLevel)
                        return new PetCreatureExperienceUpdate(characterId, creatureKey, beforeLevel, beforeLevel, beforeExp, beforeExp, false);
...
DO UPDATE SET equipped_creature_level = @level;";
                    ...
                    return new PetCreatureExperienceUpdate(
                        ...
                        beforeExp,
                        afterExp,
                        afterLevel > beforeLevel || afterExp != beforeExp);
                }
            }
        }

        public static int GetCreatureLevelForExperience(int experience)
        {
...
LIMIT 1;";
                ...
                try
                {
                    ...
                    if (values.Count > 0)
                    {
                        ...
                        for (var i = 0; i < table.Length; i++)
                            table[i] = values[i];
                        FileLogger.Log($"[PetCreatureExp] Loaded {table.Length} creature exp thresholds from {path}");
                        return table;
                    }
                }
                catch
                {
                    // Try the next casing variant; PVF archives differ in path casing.
                }
            }

            FileLogger.Log("[PetCreatureExp] WARN: creature exp table missing; using no-level-up fallback");
    ...
    public readonly struct PetCreatureExperienceUpdate
    {
        public PetCreatureExperienceUpdate(
            int characterId,
            int creatureKey,
            int beforeLevel,
            int afterLevel,
        ...
        public static PetCreatureExperienceUpdate Noop(int characterId, int creatureKey = 0)
