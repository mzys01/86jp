...
namespace DfoServer.Game.Inventory
{
    internal sealed class InventoryEnchantStore
    {
        ...
        internal InventoryEnchantStore(ScopedStoreContext context, InventoryDbPrimitives db, InventoryAuditLogger auditLogger)
        {
            _context = context;
            _db = db;
        ...
        internal bool TryEnchantByBead(EnchantByBeadCommand command, out EnchantByBeadResult result)
        {
            if (command == null)
            {
            ...
            {
                var bead = _db.LoadItemRecord(connection, transaction, InventoryListType.Main, command.BeadSlotIndex);
                ...
                var target = _db.LoadItemRecord(connection, transaction, InventoryListType.Main, command.TargetSlotIndex);
                ...
                var targetCommon = _db.LoadCommonItem(connection, transaction, InventoryListType.Main, command.TargetSlotIndex);
                ...
                if (remainingBeadCount > 0)
                {
                    ...
                    beadCommon = _db.LoadCommonItem(connection, transaction, InventoryListType.Main, command.BeadSlotIndex);
                    if (beadCommon == null)
                ...
                _auditLogger.WriteDeleteAuditLog(connection, transaction, bead, 1);
                _auditLogger.WriteEnchantAuditLog(connection, transaction, bead, target, enchantCardItemId, enchantUpgradeCount);
                transaction.Commit();
